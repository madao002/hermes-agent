...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # ── local-modifications guard (Hermes local fork) ─────────────────
    # The upstream updater assumes a pristine main checkout: it can stash
    # local changes, reset the tree, or switch branches — all of which
    # would destroy the local-modifications branch this install is pinned
    # to. Refuse to run the auto-updater on any non-main branch; updates
    # go through `git fetch upstream && git merge upstream/main`.
    try:
        _br = subprocess.run(
        ...
        if _br.returncode == 0 and _br.stdout.strip() != "main":
            print(
                f"✗ Refusing to run the auto-updater on branch "
                f"'{_br.stdout.strip()}'. This install keeps local "
                "customizations on the local-modifications branch; the "
                "updater is main-only."
            )
            print("  To update: git fetch upstream && git merge upstream/main")
            sys.exit(0)
    except Exception as _guard_err:
        logger.debug("local-modifications branch guard skipped: %s", _guard_err)
    # ── end guard ─────────────────────────────────────────────────────
